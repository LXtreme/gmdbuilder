"""Object TypedDict definitions"""

# IMPORTANT: the 'a' suffix is a workaround for type safety.
# 
# These types are autogenerated through a 3 step process. 
# Read build_tools for autogeneration scripts

from typing import Callable, Required, TypedDict, Any
import traceback

from gmdbuilder.internal_mappings.obj_id import ObjId
from src.gmdbuilder.internal_mappings.obj_prop import ObjProp


SPAWNING_TRIGGER_IDS = {
    ObjId.Trigger.SPAWN,
    ObjId.Trigger.TOGGLE,
    ObjId.Trigger.STOP,
    ObjId.Trigger.COLLISION,
    ObjId.Trigger.INSTANT_COLLISION,
    ObjId.Trigger.INSTANT_COUNT
}

GROUP_FIELDS = {
    ObjProp.GROUPS,
    ObjProp.Trigger.Spawn.GROUP_ID,
    ObjProp.Trigger.Spawn.REMAPS,
    ObjProp.Trigger.Move.TARGET_POS
}

Validator = Callable[[list["ObjectType"], list["ObjectType"]], None]


# ===== ObjectList Class =====

class ObjectList(list["ObjectType"]):
    """
    Custom list for managing Geometry Dash objects with validation and tracking.
    
    This class extends list[ObjectType] and adds:
    - Automatic validation on mutations
    - Traceback tracking for added/modified objects
    - Pattern matching for deletions
    - Tagging support for bulk operations
    """
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.tag_group: int = 9999
        self._modification_traces: dict[int, traceback.FrameSummary] = {}
    
    def append(self, obj: "ObjectType") -> None:
        """Add an object to the list with validation."""
        from gmdbuilder import validation
        
        # Immediate validation
        validation.validate_object_immediate(obj)
        
        # Track where this was added
        stack = traceback.extract_stack()
        # Find the first frame outside gmdbuilder
        for frame in reversed(stack[:-1]):
            if 'gmdbuilder' not in frame.filename:
                obj_id = id(obj)
                self._modification_traces[obj_id] = frame
                break
        
        super().append(obj)
    
    def extend(self, objects: list["ObjectType"] | tuple["ObjectType", ...]) -> None:  # type: ignore
        """Add multiple objects to the list with validation."""
        from gmdbuilder import validation
        
        for obj in objects:
            validation.validate_object_immediate(obj)
        
        # Track where these were added
        stack = traceback.extract_stack()
        for frame in reversed(stack[:-1]):
            if 'gmdbuilder' not in frame.filename:
                for obj in objects:
                    obj_id = id(obj)
                    self._modification_traces[obj_id] = frame
                break
        
        super().extend(objects)
    
    def update(self, obj: "ObjectType", patch: dict[str, Any]) -> None:
        """
        Update an object with a patch dict, validating the changes.
        
        Args:
            obj: The object to update (must be in the list)
            patch: Dictionary of properties to update
        """
        from gmdbuilder import validation
        
        # Validate each property in the patch
        for key, value in patch.items():
            validation.validate_property_immediate(obj, key, value)
        
        # Apply the patch
        obj.update(patch)  # type: ignore
        
        # Track modification
        stack = traceback.extract_stack()
        for frame in reversed(stack[:-1]):
            if 'gmdbuilder' not in frame.filename:
                obj_id = id(obj)
                self._modification_traces[obj_id] = frame
                break
    
    def delete(self, obj: "ObjectType") -> None:
        """
        Delete an object by exact match (all keys and values must match).
        
        Args:
            obj: The object to delete
            
        Raises:
            ValueError: If exact match not found
        """
        for i, existing in enumerate(self):
            if existing == obj:
                del self[i]
                return
        raise ValueError("Object not found in list (no exact match)")
    
    def delete_where(self, pattern: "dict[str, Any] | Callable[[ObjectType], bool]", limit: int | None = None) -> int:
        """
        Delete objects matching a pattern or predicate function.
        
        Args:
            pattern: Either a dict (matches if object has same keys/values) or a callable predicate
            limit: Maximum number of objects to delete (None = delete all matches)
            
        Returns:
            Number of objects deleted
        """
        deleted = 0
        i = 0
        
        while i < len(self):
            obj = self[i]
            should_delete = False
            
            if callable(pattern):
                # Predicate function
                should_delete = pattern(obj)
            else:
                # Pattern dict - check if obj has all the same keys/values
                should_delete = all(
                    obj.get(key) == value 
                    for key, value in pattern.items()
                )
            
            if should_delete:
                del self[i]
                deleted += 1
                if limit is not None and deleted >= limit:
                    break
            else:
                i += 1
        
        return deleted
    
    def delete_matches(self, pattern: dict[str, Any]) -> int:
        """
        Delete objects matching a pattern dict.
        
        Args:
            pattern: Dictionary with keys/values to match
            
        Returns:
            Number of objects deleted
        """
        return self.delete_where(pattern)
    
    def delete_tagged(self) -> int:
        """
        Delete all objects with the current tag_group.
        
        Returns:
            Number of objects deleted
        """
        from gmdbuilder.internal_mappings.obj_prop import ObjProp
        
        def has_tag(obj: "ObjectType") -> bool:
            groups = obj.get(ObjProp.GROUPS)
            if groups is None:
                return False
            # Groups can be a list or a single int
            if isinstance(groups, list):
                return self.tag_group in groups
            return groups == self.tag_group
        
        return self.delete_where(has_tag)
    
    def get_modification_trace(self, obj: "ObjectType") -> traceback.FrameSummary | None:
        """Get the traceback frame where an object was added/modified."""
        obj_id = id(obj)
        return self._modification_traces.get(obj_id)


# DO NOT EDIT. INSTEAD EDIT TYPEMAP.TSV AND REGENERATE
# ======================================================================
#   AUTOGENERATED TYPEDDICT
# ======================================================================
# Start

class ObjectType(TypedDict, total=False):
    a1: int  # ID
    a2: float  # X
    a3: float  # Y
    a4: bool  # FLIP_X
    a5: bool  # FLIP_Y
    a6: float  # ROTATION
    a19: Any  # OLD_COLOR_ID
    a20: int  # EDITOR_L1
    a21: int  # COLOR_1
    a22: int  # COLOR_2
    a24: int  # Z_LAYER
    a25: int  # Z_ORDER
    a32: float  # OLD_SCALE
    a34: bool  # GROUP_PARENT
    a41: bool  # HSV_ENABLED_1
    a42: bool  # HSV_ENABLED_2
    a43: Any  # HSV_1
    a44: Any  # HSV_2
    a57: Any  # GROUPS
    a61: int  # EDITOR_L2
    a64: bool  # DONT_FADE
    a67: bool  # DONT_ENTER
    a96: bool  # NO_GLOW
    a103: bool  # HIGH_DETAIL
    a108: int  # LINKED_GROUP
    a116: bool  # NO_EFFECTS
    a121: bool  # NO_TOUCH
    a128: float  # SCALE_X
    a129: float  # SCALE_Y
    a131: float  # SKEW_X
    a132: float  # SKEW_Y
    a134: bool  # PASSABLE
    a135: bool  # HIDE
    a136: bool  # NONSTICK_X
    a137: bool  # ICE_BLOCK
    a155: int  # COLOR_1_INDEX
    a156: int  # COLOR_2_INDEX
    a193: bool  # GRIP_SLOPE
    a200: bool  # TARGET_PLAYER_2
    a274: Any  # PARENT_GROUPS
    a279: bool  # AREA_PARENT
    a289: bool  # NONSTICK_Y
    a343: int  # ENTER_CHANNEL
    a356: bool  # SCALE_STICK
    a370: bool  # DISABLE_GRID_SNAP
    a372: bool  # NO_AUDIO_SCALE
    a446: int  # MATERIAL
    a495: bool  # EXTRA_STICKY
    a496: bool  # DONT_BOOST_Y
    a497: Any  # SINGLE_COLOR_TYPE
    a507: bool  # NO_PARTICLE
    a509: bool  # DONT_BOOST_X
    a511: bool  # EXTENDED_COLLISION

class AnimatedType(ObjectType, total=False):
    a106: bool  # RANDOMIZE_START
    a107: float  # ANIMATION_SPEED
    a122: bool  # USE_SPEED
    a123: bool  # ANIMATE_ON_TRIGGER
    a126: bool  # DELAYED_LOOP
    a214: bool  # ANIMATE_ACTIVE_ONLY
    a462: int  # SINGLE_FRAME
    a592: bool  # OFFSET_ANIM

class ExplosionType(AnimatedType, total=False):
    a127: Required[bool]  # DISABLE_SHINE

class ItemLabelType(ObjectType, total=False):
    a80: int  # ITEM_ID
    a389: bool  # SECONDS_ONLY
    a390: Any  # SPECIAL_ID
    a391: Any  # ALIGNMENT
    a466: bool  # TIME_COUNTER
    a488: int  # KERNING

class LevelType(ObjectType, total=False):
    pass

class Color17Type(LevelType, total=False):
    pass

class BackgroundType(Color17Type, total=False):
    pass

class GroundType(Color17Type, total=False):
    pass

class LineType(Color17Type, total=False):
    pass

class Object2Type(Color17Type, total=False):
    pass

class Color19Type(LevelType, total=False):
    pass

class ParticleType(ObjectType, total=False):
    a145: Any  # DATA
    a146: bool  # USE_OBJ_COLOR
    a147: bool  # UNIFORM_OBJ_COLOR
    a211: bool  # QUICK_START

class SawType(ObjectType, total=False):
    a97: int  # ROTATION_SPEED
    a98: bool  # DISABLE_ROTATION

class StartPosType(ObjectType, total=False):
    pass

class TemplateType(ObjectType, total=False):
    a157: bool  # REFERENCE_ONLY

class TextType(ObjectType, total=False):
    a31: Any  # DATA
    a488: int  # KERNING

class TimewarpType(ObjectType, total=False):
    a120: float  # MOD

class TriggerType(ObjectType, total=False):
    a11: bool  # TOUCH_TRIGGER
    a13: bool  # EDITOR_PREVIEW
    a36: bool  # INTERACTIBLE
    a62: bool  # SPAWN_TRIGGER
    a87: bool  # MULTI_TRIGGER
    a99: bool  # MULTI_ACTIVATE
    a115: int  # ORDER
    a117: bool  # REVERSE
    a170: int  # CHANNEL
    a280: bool  # IGNORE_GPARENT
    a281: bool  # IGNORE_LINKED
    a284: bool  # SINGLE_PTOUCH
    a369: bool  # CENTER_EFFECT
    a444: bool  # DISABLE_MULTI_ACTIVATE
    a534: Any  # CONTROL_ID

class AdvFollowType(TriggerType, total=False):
    a51: int  # TARGET_ID
    a71: int  # FOLLOW_ID
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a201: bool  # CORNER
    a292: float  # DELAY
    a293: float  # DELAY_RAND
    a298: float  # MAX_SPEED
    a299: float  # MAX_SPEED_RAND
    a300: float  # START_SPEED
    a301: float  # START_SPEED_RAND
    a305: bool  # TARGET_DIR
    a306: bool  # X_ONLY
    a307: bool  # Y_ONLY
    a308: int  # MAX_RANGE
    a309: int  # MAX_RANGE_RAND
    a310: Any  # M_310
    a311: Any  # M_311
    a312: Any  # M_312
    a313: Any  # M_313
    a314: Any  # M_314
    a315: Any  # M_315
    a316: float  # STEER
    a317: float  # STEER_RAND
    a318: float  # STEER_LOW
    a319: float  # STEER_LOW_RAND
    a320: float  # STEER_HIGH
    a321: float  # STEER_HIGH_RAND
    a322: float  # SPEED_RANGE_LOW
    a323: float  # SPEED_RANGE_LOW_RAND
    a324: float  # SPEED_RANGE_HIGH
    a325: float  # SPEED_RANGE_HIGH_RAND
    a326: float  # BREAK_FORCE
    a327: float  # BREAK_FORCE_RAND
    a328: int  # BREAK_ANGLE
    a329: int  # BREAK_ANGLE_RAND
    a330: float  # BREAK_STEER
    a331: float  # BREAK_STEER_RAND
    a332: float  # BREAK_STEER_SPEED_LIMIT
    a333: float  # BREAK_STEER_SPEED_LIMIT_RAND
    a334: float  # ACCELERATION
    a335: float  # ACCELERATION_RAND
    a336: bool  # IGNORE_DISABLED
    a337: bool  # STEER_LOW_CHECK
    a338: bool  # STEER_HIGH_CHECK
    a339: bool  # ROTATE_DIR
    a340: int  # ROT_OFFSET
    a357: float  # NEAR_ACCEL
    a358: float  # NEAR_ACCEL_RAND
    a359: float|int  # NEAR_DIST
    a360: float|int  # NEAR_DIST_RAND
    a361: float  # EASING
    a362: float  # EASING_RAND
    a363: int  # ROT_EASING
    a364: int  # ROT_DEADZONE
    a365: int  # PRIORITY
    a366: int  # MAX_RANGE_REF
    a367: Any  # MODE
    a558: float  # FRICTION
    a559: float  # FRICTION_RAND
    a560: int  # START_SPEED_REF
    a561: float  # NEAR_FRICTION
    a562: float  # NEAR_FRICTION_RAND
    a563: int  # START_DIR
    a564: int  # START_DIR_RAND
    a565: int  # START_DIR_REF
    a571: bool  # EXCLUSIVE
    a572: Any  # INIT

class AdvRandomType(TriggerType, total=False):
    a56: bool  # m_56
    a152: Any  # TARGETS

class AlphaType(TriggerType, total=False):
    a10: float  # DURATION
    a35: float  # OPACITY
    a51: int  # GROUP_ID

class AnimateType(TriggerType, total=False):
    a51: int  # TARGET_ID
    a76: int  # ANIMATION_ID

class AnimateKeyframeType(TriggerType, total=False):
    a51: int  # TARGET_ID
    a71: int  # PARENT_ID
    a76: int  # ANIMATION_ID
    a520: float  # TIME_MOD
    a521: float  # POS_X_MOD
    a522: float  # ROTATION_MOD
    a523: float  # SCALE_X_MOD
    a545: float  # POS_Y_MOD
    a546: float  # SCALE_Y_MOD

class ArrowType(TriggerType, total=False):
    a166: Any  # DIR_Y_NEG
    a167: Any  # DIR_X_POS
    a169: bool  # EDIT_VELOCITY
    a171: bool  # CHANGE_CHANNEL
    a172: bool  # CHANNEL_ONLY
    a173: Any  # TARGET_CHANNEL
    a368: bool  # INSTANT_OFFSET
    a582: float  # VELOCITY_MOD_X
    a583: float  # VELOCITY_MOD_Y
    a584: bool  # OVERRIDE_VELOCITY
    a585: bool  # DONT_SLIDE

class BgSpeedType(TriggerType, total=False):
    a143: float  # X_MOD
    a144: float  # Y_MOD

class BpmType(TriggerType, total=False):
    a10: float  # DURATION
    a498: int  # BPM
    a499: Any  # SPEED
    a500: bool  # DISABLE
    a501: int  # BPB

class CameraEdgeType(TriggerType, total=False):
    a51: int  # TARGET_ID
    a164: Any  # DIRECTION

class CameraGuideType(TriggerType, total=False):
    a28: int  # OFFSET_X
    a29: int  # OFFSET_Y
    a371: float  # ZOOM
    a506: float  # PREVIEW_OPACITY

class CameraModeType(TriggerType, total=False):
    a111: bool  # FREE_MODE
    a112: bool  # EDIT_SETTINGS
    a113: int  # EASING
    a114: float  # PADDING

class ChangeBgType(TriggerType, total=False):
    a533: Any  # BG_ID

class ChangeGrType(TriggerType, total=False):
    a533: Any  # GR_ID

class ChangeMgType(TriggerType, total=False):
    a533: Any  # MG_ID

class CheckpointType(TriggerType, total=False):
    a51: int  # SPAWN_ID
    a71: int  # TARGET_POS
    a138: bool  # PLAYER_POS
    a448: Any  # RESPAWN_ID

class CollectibleType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a78: bool  # SUB_COUNT
    a80: int  # ITEM_ID
    a381: bool  # PICKUP_ITEM
    a382: bool  # TOGGLE_TRIGGER
    a383: int  # POINTS
    a440: int  # PARTICLE
    a463: bool  # NO_ANIM

class CoinType(CollectibleType, total=False):
    a12: int  # COIN_ID

class CollisionType(TriggerType, total=False):
    a10: float  # m_10
    a51: int  # TARGET_ID
    a56: bool  # ACTIVATE_GROUP
    a80: int  # BLOCK_A
    a93: bool  # EXIT
    a95: Any  # BLOCK_B
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a201: bool  # BETWEEN_PLAYERS

class CollisionBlockType(TriggerType, total=False):
    a80: int  # BLOCK_ID
    a94: bool  # DYNAMIC

class ColorType(TriggerType, total=False):
    a7: int  # RED
    a8: int  # GREEN
    a9: int  # BLUE
    a10: float  # DURATION
    a14: bool  # TINT_GROUND
    a15: bool  # PLAYER_1
    a16: bool  # PLAYER_2
    a17: bool  # BLENDING
    a23: Any  # CHANNEL
    a35: float  # OPACITY
    a49: Any  # HSV
    a50: Any  # COPY_ID
    a60: bool  # COPY_OPACITY
    a210: bool  # DISABLE_LEGACY_HSV

class CountType(TriggerType, total=False):
    a51: int  # TARGET_ID
    a56: bool  # ACTIVATE_GROUP
    a77: int  # COUNT
    a80: int  # ITEM_ID
    a104: bool  # MULTI_ACTIVATE

class DashType(TriggerType, total=False):
    a586: float  # SPEED
    a587: bool  # COLLIDE
    a588: float  # END_BOOST
    a589: bool  # STOP_SLIDE
    a590: float  # MAX_DURATION

class EditAdvFollowType(TriggerType, total=False):
    a51: int  # TARGET_ID
    a71: int  # FOLLOW_ID
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a201: bool  # CORNER
    a292: float  # M_292
    a293: float  # M_293
    a298: float  # M_298
    a299: float  # M_299
    a300: float  # SPEED
    a301: float  # SPEED_RAND
    a305: bool  # M_305
    a306: bool  # X_ONLY
    a307: bool  # Y_ONLY
    a308: int  # M_308
    a309: int  # M_309
    a310: Any  # M_310
    a311: Any  # M_311
    a312: Any  # M_312
    a313: Any  # M_313
    a314: Any  # M_314
    a315: Any  # M_315
    a316: float  # M_316
    a317: float  # M_317
    a318: float  # M_318
    a319: float  # M_319
    a320: float  # M_320
    a321: float  # M_321
    a322: float  # M_322
    a323: float  # M_323
    a324: float  # M_324
    a325: float  # M_325
    a326: float  # M_326
    a327: float  # M_327
    a328: int  # M_328
    a329: int  # M_329
    a330: float  # M_330
    a331: float  # M_331
    a332: float  # M_332
    a333: float  # M_333
    a334: float  # M_334
    a335: float  # M_335
    a336: bool  # M_336
    a337: bool  # M_337
    a338: bool  # M_338
    a339: bool  # M_339
    a340: int  # M_340
    a357: float  # M_357
    a358: float  # M_358
    a359: float|int  # M_359
    a360: float|int  # M_360
    a361: float  # M_361
    a362: float  # M_362
    a363: int  # M_363
    a364: int  # M_364
    a365: int  # M_365
    a366: int  # M_366
    a367: Any  # M_367
    a535: bool  # USE_CONTROL_ID
    a558: float  # M_558
    a559: float  # M_559
    a560: int  # SPEED_REF
    a561: float  # M_561
    a562: float  # M_562
    a563: int  # DIR
    a564: int  # DIR_RAND
    a565: int  # DIR_REF
    a566: float  # MOD_X
    a567: float  # MOD_X_RAND
    a568: float  # MOD_Y
    a569: float  # MOD_Y_RAND
    a570: bool  # REDIRECT_DIR
    a571: bool  # M_571
    a572: Any  # M_572

class EffectType(TriggerType, total=False):
    a10: float  # DURATION
    a49: Any  # HSV
    a51: int  # TARGET_ID
    a65: bool  # MAIN_ONLY
    a66: bool  # DETAIL_ONLY
    a71: int  # CENTER_ID
    a138: bool  # m_138
    a200: bool  # m_200
    a201: bool  # m_201
    a217: Any  # ENTER_ONLY
    a218: int  # MOVE_DIST
    a219: int  # MOVE_DIST_RAND
    a220: int  # OFFSET
    a221: int  # OFFSET_RAND
    a222: int  # LENGTH
    a223: int  # LENGTH_RAND
    a224: Any  # m_224
    a225: Any  # EFFECT_ID
    a231: int  # MOVE_ANGLE
    a232: int  # MOVE_ANGLE_RAND
    a233: float  # SCALE_X
    a234: float  # SCALE_X_RAND
    a235: float  # SCALE_Y
    a236: float  # SCALE_Y_RAND
    a237: int  # MOVE_X
    a238: int  # MOVE_X_RAND
    a239: int  # MOVE_Y
    a240: int  # MOVE_Y_RAND
    a241: bool  # XY_MODE
    a242: Any  # EASING
    a243: float  # EASING_RATE
    a248: Any  # EASING_2
    a249: float  # EASING_RATE_2
    a252: int  # OFFSET_Y
    a253: int  # OFFSET_Y_RAND
    a260: Any  # TINT_CHANNEL
    a261: bool  # EASE_OUT
    a262: Any  # DIRECTION
    a263: float  # MOD_FRONT
    a264: float  # MOD_BACK
    a265: float  # TINT
    a270: float  # ROTATE
    a271: float  # ROTATE_RAND
    a275: float  # TO_OPACITY
    a276: bool  # INWARDS
    a278: bool  # ENABLE_HSV
    a282: float  # DEADZONE
    a283: bool  # MIRRORED
    a285: float  # M_285
    a286: float  # FROM_OPACITY
    a287: bool  # RELATIVE
    a288: float  # RFADE
    a341: int  # PRIORITY
    a344: int  # ENTER_CHANNEL
    a355: Any  # USE_EFFECT_ID
    a538: Any  # SPECIAL_CENTER
    a539: bool  # DEAP

class EndType(TriggerType, total=False):
    a51: int  # SPAWN_ID
    a71: int  # TARGET_POS
    a460: bool  # NO_EFFECTS
    a461: bool  # NO_SFX
    a487: bool  # INSTANT

class EndWallType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a59: bool  # LOCK_Y
    a118: bool  # REVERSE

class EnterPresetType(TriggerType, total=False):
    a217: Any  # ENTER_ONLY
    a344: int  # ENTER_CHANNEL

class EventType(TriggerType, total=False):
    a51: int  # SPAWN_ID
    a430: Any  # EVENTS
    a431: Any  # M_431
    a447: int  # EXTRA_ID_1
    a525: int  # EXTRA_ID_2

class FollowType(TriggerType, total=False):
    a10: float  # DURATION
    a51: int  # TARGET_ID
    a71: int  # FOLLOW_TARGET
    a72: float  # MOD_X
    a73: float  # MOD_Y

class FollowPlayerYType(TriggerType, total=False):
    a10: float  # DURATION
    a51: int  # TARGET_ID
    a90: float  # SPEED
    a91: float  # DELAY
    a92: int  # OFFSET
    a105: float  # MAX_SPEED

class ForceBlockType(TriggerType, total=False):
    a149: float  # VALUE
    a526: float  # VALUE_MIN
    a527: float  # VALUE_MAX
    a528: bool  # RELATIVE
    a529: bool  # RANGE
    a530: Any  # FORCE_ID

class GamemodePortalType(TriggerType, total=False):
    a111: bool  # FREE_MODE
    a112: bool  # EDIT_SETTINGS
    a113: int  # EASING
    a114: float  # PADDING

class GameplayOffsetType(TriggerType, total=False):
    a28: int  # OFFSET_X
    a29: int  # OFFSET_Y
    a58: bool  # DONT_ZOOM_X
    a59: bool  # DONT_ZOOM_Y

class GradientType(TriggerType, total=False):
    a174: Any  # BLENDING
    a202: Any  # LAYER
    a203: int  # U (ALIASES: BL)
    a204: int  # D (ALIASES: BR)
    a205: int  # L (ALIASES: TL)
    a206: int  # R (ALIASES: TR)
    a207: bool  # VERTEX_MODE
    a208: bool  # DISABLE
    a209: int  # GRADIENT_ID
    a456: float  # PREVIEW_OPACITY
    a508: bool  # DISABLE_ALL

class GravityType(TriggerType, total=False):
    a138: bool  # PLAYER_1
    a148: float  # VALUE
    a200: bool  # PLAYER_2
    a201: bool  # PLAYER_TOUCH

class InstantCollisionType(TriggerType, total=False):
    a51: int  # TRUE_ID
    a71: int  # FALSE_ID
    a80: int  # BLOCK_A
    a95: Any  # BLOCK_B
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a201: bool  # BETWEEN_PLAYERS

class InstantCountType(TriggerType, total=False):
    a51: Required[int]  # TARGET_ID
    a56: bool  # ACTIVATE_GROUP
    a77: int  # COUNT
    a80: int  # ITEM_ID
    a88: Any  # MODE

class ItemCompareType(TriggerType, total=False):
    a51: int  # TRUE_ID
    a71: int  # FALSE_ID
    a80: int  # ITEM_ID_1
    a95: Any  # ITEM_ID_2
    a476: Any  # ITEM_TYPE_1
    a477: Any  # ITEM_TYPE_2
    a479: float  # MOD_1
    a480: Any  # ITEM_OP_1
    a481: Any  # ITEM_OP_2
    a482: Any  # ITEM_OP_3
    a483: float  # MOD_2
    a484: float  # TOLERANCE
    a485: Any  # ROUND_OP_1
    a486: Any  # ROUND_OP_2
    a578: Any  # SIGN_OP_1
    a579: Any  # SIGN_OP_2

class ItemEditType(TriggerType, total=False):
    a51: int  # TARGET_ITEM_ID
    a80: int  # ITEM_ID_1
    a95: Any  # ITEM_ID_2
    a476: Any  # ITEM_TYPE_1
    a477: Any  # ITEM_TYPE_2
    a478: Any  # ITEM_TYPE_3
    a479: float  # MOD
    a480: Any  # ITEM_OP_1
    a481: Any  # ITEM_OP_2
    a482: Any  # ITEM_OP_3
    a485: Any  # ROUND_OP_1
    a486: Any  # ROUND_OP_2
    a578: Any  # SIGN_OP_1
    a579: Any  # SIGN_OP_2

class ItemPersistType(TriggerType, total=False):
    a80: int  # ITEM_ID
    a491: bool  # SET_PERSISTENT
    a492: bool  # TARGET_ALL
    a493: bool  # RESET
    a494: bool  # TIMER

class KeyframeType(TriggerType, total=False):
    a10: float  # DURATION
    a30: Any  # EASING
    a51: int  # GROUP_ID
    a71: int  # SPAWN_ID
    a85: float  # EASE_RATE
    a373: int  # KEY_ID
    a374: int  # INDEX
    a375: bool  # REF_ONLY
    a376: bool  # CLOSE_LOOP
    a377: bool  # PROX
    a378: bool  # CURVE
    a379: Any  # TIME_MODE
    a380: bool  # PREVIEW_ART
    a459: bool  # AUTO_LAYER
    a524: float  # LINE_OPACITY
    a536: Any  # SPIN_DIRECTION
    a537: int  # FULL_ROTATIONS
    a557: float  # SPAWN_DELAY

class LinkVisibleType(TriggerType, total=False):
    a51: int  # GROUP_ID

class MgEditType(TriggerType, total=False):
    a10: float  # DURATION
    a29: int  # OFFSET_Y
    a30: Any  # EASING
    a85: float  # EASE_RATE

class MgSpeedType(TriggerType, total=False):
    a143: float  # X_MOD
    a144: float  # Y_MOD

class MoveType(TriggerType, total=False):
    a10: float  # DURATION
    a28: int  # MOVE_X
    a29: int  # MOVE_Y
    a30: Any  # EASING
    a51: Required[int]  # TARGET_ID
    a58: bool  # LOCK_PLAYER_X
    a59: bool  # LOCK_PLAYER_Y
    a71: int  # TARGET_POS
    a85: float  # EASE_RATE
    a100: bool  # TARGET_MODE
    a101: Any  # TARGET_AXIS
    a138: bool  # PLAYER_1
    a141: bool  # LOCK_CAMERA_X
    a142: bool  # LOCK_CAMERA_Y
    a143: float  # FOLLOW_X_MOD
    a144: float  # FOLLOW_Y_MOD
    a200: bool  # PLAYER_2
    a393: bool  # USE_SMALL_STEP
    a394: bool  # DIRECTION_MODE
    a395: Any  # TARGET_CENTER_ID
    a396: int  # TARGET_DISTANCE
    a397: bool  # DYNAMIC_MODE
    a516: Any  # M_516
    a517: Any  # M_517
    a518: Any  # M_518
    a519: Any  # M_519
    a544: bool  # SILENT

class ObjectControlType(TriggerType, total=False):
    a51: int  # TARGET_ID

class OffsetCameraType(TriggerType, total=False):
    a28: int  # OFFSET_X
    a29: int  # OFFSET_Y
    a30: Any  # EASING
    a85: float  # EASE_RATE
    a101: Any  # AXIS

class OffsetGameplayType(TriggerType, total=False):
    a101: Any  # AXIS

class OnDeathType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a56: bool  # ACTIVATE_GROUP

class OptionsType(TriggerType, total=False):
    a159: Any  # STREAK_ADDITIVE
    a160: Any  # UNLINK_DUAL_GRAVITY
    a161: Any  # HIDE_GROUND
    a162: Any  # HIDE_P1
    a163: Any  # HIDE_P2
    a165: Any  # DISABLE_P1_CONTROLS
    a195: Any  # HIDE_MG
    a199: Any  # DISABLE_CONTROLS_P1
    a532: Any  # HIDE_ATTEMPTS
    a573: Any  # EDIT_RESPAWN_TIME
    a574: float  # RESPAWN_TIME
    a575: Any  # AUDIO_ON_DEATH
    a576: Any  # DISABLE_DEATH_SFX
    a593: Any  # BOOST_SLIDE

class OrbSawType(TriggerType, total=False):
    a97: int  # ROTATION_SPEED
    a98: bool  # DISABLE_ROTATION

class PickupType(TriggerType, total=False):
    a77: int  # COUNT
    a80: int  # ITEM_ID
    a88: Any  # MODE
    a139: bool  # OVERRIDE
    a449: float  # MOD

class PlayerControlType(TriggerType, total=False):
    a58: bool  # m_58
    a59: bool  # m_59
    a138: bool  # PLAYER_1
    a141: bool  # m_141
    a200: bool  # PLAYER_2
    a540: bool  # STOP_JUMP
    a541: bool  # STOP_MOVE
    a542: bool  # STOP_ROTATION
    a543: bool  # STOP_SLIDE

class PulseType(TriggerType, total=False):
    a7: int  # RED
    a8: int  # GREEN
    a9: int  # BLUE
    a45: float  # FADE_IN
    a46: float  # HOLD
    a47: float  # FADE_OUT
    a48: bool  # USE_HSV
    a49: Any  # HSV
    a50: Any  # COPY_ID
    a51: Required[int]  # TARGET_ID
    a52: bool  # TARGET_TYPE
    a65: bool  # MAIN_ONLY
    a66: bool  # DETAIL_ONLY
    a86: bool  # EXCLUSIVE
    a210: bool  # DISABLE_STATIC_HSV

class RandomType(TriggerType, total=False):
    a10: float  # CHANCE
    a51: int  # TRUE_ID
    a71: int  # FALSE_ID

class ResetType(TriggerType, total=False):
    a51: int  # GROUP_ID

class RotateType(TriggerType, total=False):
    a10: float  # DURATION
    a30: Any  # EASING
    a51: Required[int]  # TARGET_ID
    a68: float  # DEGREES
    a69: int  # FULL
    a70: bool  # LOCK_ROTATION
    a71: int  # CENTER_ID
    a85: float  # EASE_RATE
    a100: bool  # AIM_MODE
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a394: bool  # FOLLOW_MODE
    a397: bool  # DYNAMIC_MODE
    a401: int  # AIM_TARGET
    a402: float  # AIM_OFFSET
    a403: int  # AIM_EASING
    a516: Any  # MIN_X_ID
    a517: Any  # MIN_Y_ID
    a518: Any  # MAX_X_ID
    a519: Any  # MAX_Y_ID

class RotateCameraType(TriggerType, total=False):
    a10: float  # DURATION
    a30: Any  # EASING
    a68: float  # DEGREES
    a70: bool  # ADD
    a85: float  # EASE_RATE
    a394: bool  # SNAP_360

class ScaleType(TriggerType, total=False):
    a10: float  # DURATION
    a30: Any  # EASING
    a51: Required[int]  # TARGET_ID
    a71: int  # CENTER_ID
    a85: float  # EASE_RATE
    a133: bool  # ONLY_MOVE
    a150: float  # SCALE_BY_X
    a151: float  # SCALE_BY_Y
    a153: bool  # DIV_BY_X
    a154: bool  # DIV_BY_Y
    a452: bool  # RELATIVE_ROTATION
    a577: bool  # RELATIVE_SCALE

class SequenceType(TriggerType, total=False):
    a435: Any  # SEQUENCE
    a436: Any  # MODE
    a437: float  # MIN_INTERVAL
    a438: float  # RESET_TIME
    a439: bool  # RESET_TYPE
    a505: bool  # UNIQUE_REMAP

class SfxType(TriggerType, total=False):
    a10: float  # DURATION
    a51: int  # GROUP_ID_1
    a71: int  # GROUP_ID_2
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a392: int  # SFX_ID
    a404: int  # SPEED
    a405: int  # PITCH
    a406: float  # VOLUME
    a407: bool  # USE_REVERB
    a408: int  # START
    a409: int  # FADE_IN
    a410: int  # END
    a411: int  # FADE_OUT
    a412: bool  # FFT
    a413: bool  # LOOP
    a417: bool  # STOP_LOOP
    a415: bool  # UNIQUE
    a416: Any  # UNIQUE_ID
    a418: bool  # CHANGE_VOLUME
    a419: bool  # CHANGE_SPEED
    a420: bool  # OVERRIDE
    a421: float  # VOL_NEAR
    a422: float  # VOL_MED
    a423: float  # VOL_FAR
    a424: int  # DIST_1
    a425: int  # DIST_2
    a426: int  # DIST_3
    a428: bool  # CAMERA
    a433: bool  # PRE_LOAD
    a434: float  # MIN_INT
    a455: Any  # GROUP
    a457: Any  # GROUP_ID
    a458: Any  # DIRECTION
    a489: bool  # IGNORE_VOLUME
    a490: float  # SFX_DURATION
    a502: Any  # REVERB
    a503: bool  # OVERRIDE_REVERB
    a595: bool  # M_595
    a596: int  # SPEED_RAND
    a597: int  # PITCH_RAND
    a598: int  # VOLUME_RAND
    a599: bool  # PITCH_STEPS

class ShaderType(TriggerType, total=False):
    a10: float  # FADE_TIME
    a30: Any  # EASING
    a51: int  # SHOCKWAVE_CENTER_ID (ALIASES: SHOCKLINE_CENTER_ID, LENS_CIRCLE_CENTER_ID, RADIAL_BLUR_CENTER_ID, MOTION_BLUR_CENTER_ID, BULGE_CENTER_ID, PINCH_CENTER_ID, GRAY_SCALE_TINT_CHANNEL)
    a71: int  # LENS_CIRCLE_TINT_CHANNEL (ALIASES: RADIAL_BLUR_REF_CHANNEL, MOTION_BLUR_REF_CHANNEL)
    a85: float  # EASE_RATE
    a138: bool  # SHOCKWAVE_PLAYER_1 (ALIASES: SHOCKLINE_PLAYER_1, LENS_CIRCLE_PLAYER_1, RADIAL_BLUR_PLAYER_1, MOTION_BLUR_PLAYER_1, BULGE_PLAYER_1, PINCH_PLAYER_1)
    a175: float  # SHOCKWAVE_SPEED (ALIASES: SHOCKLINE_SPEED, GLITCH_SPEED, CHROMATIC_GLITCH_SPEED, EDIT_COLOR_CB)
    a176: float  # SHOCKWAVE_STRENGTH (ALIASES: SHOCKLINE_STRENGTH, GLITCH_STRENGTH, CHROMATIC_GLITCH_STRENGTH, LENS_CIRCLE_STRENGTH, RADIAL_BLUR_INTENSITY, MOTION_BLUR_INTENSITY, BULGE_BULGE, GRAY_SCALE_TARGET, SEPIA_TARGET, INVERT_COLOR_TARGET, HUE_DEGREES, EDIT_COLOR_CR)
    a177: float  # SHOCKWAVE_TIME_OFFSET (ALIASES: SHOCKLINE_TIME_OFFSET)
    a179: float  # SHOCKWAVE_WAVE_WIDTH (ALIASES: SHOCKLINE_WAVE_WIDTH, GLITCH_SLICE_HEIGHT, CHROMATIC_GLITCH_LINE_THICKNESS, LENS_CIRCLE_SIZE, RADIAL_BLUR_SIZE, PINCH_MODIFIER, INVERT_COLOR_R, EDIT_COLOR_BR)
    a180: float  # SHOCKWAVE_THICKNESS (ALIASES: SHOCKLINE_THICKNESS, CHROMATIC_TARGET_X, CHROMATIC_GLITCH_RGB_OFFSET, PIXELATE_TARGET_X, MOTION_BLUR_TARGET_X, BULGE_RADIUS, PINCH_TARGET_X, INVERT_COLOR_G, EDIT_COLOR_BG, SPLIT_SCREEN_TARGET_X)
    a181: float  # SHOCKWAVE_FADE_IN (ALIASES: SHOCKLINE_FADE_IN, GLITCH_MAX_COL_X_OFFSET, LENS_CIRCLE_FADE, RADIAL_BLUR_FADE, MOTION_BLUR_FADE)
    a182: float  # SHOCKWAVE_FADE_OUT (ALIASES: SHOCKLINE_FADE_OUT, GLITCH_MAX_COL_Y_OFFSET)
    a183: float  # SHOCKWAVE_INNER
    a184: bool  # SHOCKWAVE_INVERT (ALIASES: SHOCKLINE_INVERT)
    a185: bool  # SHOCKLINE_FLIP
    a186: bool  # SHOCKLINE_ROTATE
    a187: bool  # SHOCKLINE_DUAL
    a188: bool  # SHADER_OPT_IGNORE_PLAYER_PARTICLES (ALIASES: SHOCKWAVE_TARGET, SHOCKLINE_TARGET, CHROMATIC_USE_X, PIXELATE_USE_X, RADIAL_BLUR_TARGET, MOTION_BLUR_USE_X, BULGE_TARGET, PINCH_TARGET, GRAY_SCALE_USE_LUM, INVERT_COLOR_EDIT_RGB, SPLIT_SCREEN_USE_X)
    a189: float  # CHROMATIC_TARGET_Y (ALIASES: CHROMATIC_GLITCH_SEGMENT_H, PIXELATE_TARGET_Y, MOTION_BLUR_TARGET_Y, INVERT_COLOR_B, EDIT_COLOR_BB, SPLIT_SCREEN_TARGET_Y)
    a190: bool  # SHOCKWAVE_FOLLOW (ALIASES: SHOCKLINE_FOLLOW, CHROMATIC_USE_Y, PIXELATE_USE_Y, MOTION_BLUR_USE_Y, PINCH_USE_X, GRAY_SCALE_USE_TINT, INVERT_COLOR_TWEEN_RGB, SPLIT_SCREEN_USE_Y)
    a191: float  # SHOCKWAVE_OUTER (ALIASES: GLITCH_MAX_SLICE_X_OFFSET, CHROMATIC_GLITCH_LINE_STRENGTH, MOTION_BLUR_FOLLOW_EASE, EDIT_COLOR_CG)
    a192: bool  # SHADER_OPT_DISABLE_ALL (ALIASES: CHROMATIC_GLITCH_DISABLE)
    a194: bool  # CHROMATIC_GLITCH_RELATIVE_POS (ALIASES: PIXELATE_SNAP_GRID, MOTION_BLUR_DUAL_DIR, PINCH_USE_Y, INVERT_COLOR_CLAMP_RGB)
    a196: Any  # SHADER_OPT_LAYER_MIN
    a197: Any  # SHADER_OPT_LAYER_MAX
    a200: bool  # SHOCKWAVE_PLAYER_2 (ALIASES: SHOCKLINE_PLAYER_2, LENS_CIRCLE_PLAYER_2, RADIAL_BLUR_PLAYER_2, MOTION_BLUR_PLAYER_2, BULGE_PLAYER_2, PINCH_PLAYER_2)
    a201: bool  # MOTION_BLUR_CENTER
    a290: float  # SHOCKWAVE_SCREEN_OFFSET_X (ALIASES: SHOCKLINE_SCREEN_OFFSET, LENS_CIRCLE_SCREEN_OFFSET_X, RADIAL_BLUR_SCREEN_OFFSET_X, BULGE_SCREEN_OFFSET_X, PINCH_SCREEN_OFFSET_X)
    a291: float  # SHOCKWAVE_SCREEN_OFFSET_Y (ALIASES: LENS_CIRCLE_SCREEN_OFFSET_Y, RADIAL_BLUR_SCREEN_OFFSET_Y, BULGE_SCREEN_OFFSET_Y, PINCH_SCREEN_OFFSET_Y)
    a512: float  # SHOCKWAVE_MAX_SIZE (ALIASES: SHOCKLINE_MAX_SIZE, PINCH_RADIUS)
    a513: bool  # SHOCKWAVE_ANIMATE (ALIASES: SHOCKLINE_ANIMATE)
    a514: bool  # SHOCKWAVE_RELATIVE (ALIASES: SHOCKLINE_RELATIVE, GLITCH_RELATIVE, CHROMATIC_RELATIVE, CHROMATIC_GLITCH_RELATIVE, PIXELATE_RELATIVE, LENS_CIRCLE_RELATIVE, MOTION_BLUR_RELATIVE, BULGE_RELATIVE, RELATIVE)
    a515: bool  # PIXELATE_HARD_EDGES (ALIASES: RADIAL_BLUR_EMPTY_ONLY, MOTION_BLUR_EMPTY_ONLY)
    a531: bool  # DISABLE_PREVIEW

class ShakeType(TriggerType, total=False):
    a10: float  # DURATION
    a75: float  # STRENGTH
    a84: float  # INTERVAL

class SongType(TriggerType, total=False):
    a10: float  # DURATION
    a51: int  # GROUP_ID_1
    a71: int  # GROUP_ID_2
    a138: bool  # PLAYER_1
    a200: bool  # PLAYER_2
    a392: int  # SONG_ID
    a399: bool  # PREP
    a400: bool  # LOAD_PREP
    a404: int  # SPEED
    a405: int  # M_405
    a406: float  # VOLUME
    a407: bool  # M_407
    a408: int  # START
    a409: int  # FADE_IN
    a410: int  # END
    a411: int  # FADE_OUT
    a412: bool  # M_412
    a413: bool  # LOOP
    a417: bool  # STOP_LOOP
    a415: bool  # M_415
    a416: Any  # M_416
    a418: bool  # CHANGE_VOLUME
    a419: bool  # CHANGE_SPEED
    a420: bool  # M_420
    a421: float  # VOL_NEAR
    a422: float  # VOL_MED
    a423: float  # FOL_VAR
    a424: int  # DIST_1
    a425: int  # DIST_2
    a426: int  # DIST_3
    a428: bool  # CAMERA
    a432: int  # CHANNEL
    a433: bool  # M_433
    a434: float  # M_434
    a455: Any  # M_455
    a457: Any  # M_457
    a458: Any  # DIRECTION
    a489: bool  # M_489
    a490: float  # M_490
    a502: Any  # M_502
    a503: bool  # M_503
    a595: bool  # DONT_RESET
    a596: int  # M_596
    a597: int  # M_597
    a598: int  # M_598
    a599: bool  # M_599

class SpawnType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a63: float  # DELAY
    a102: bool  # DISABLE_PREVIEW
    a441: bool  # ORDERED
    a442: Any  # REMAPS
    a556: float  # DELAY_RAND
    a581: bool  # RESET_REMAP

class SpawnParticleType(TriggerType, total=False):
    a51: int  # PARTICLE_GROUP
    a71: int  # POSITION_GROUP
    a547: int  # OFFSET_X
    a548: int  # OFFSET_Y
    a549: int  # OFFVAR_X
    a550: int  # OFFVAR_Y
    a551: bool  # MATCH_ROT
    a552: int  # ROTATION
    a553: int  # ROTATION_RAND
    a554: float  # SCALE
    a555: float  # SCALE_RAND

class StateBlockType(TriggerType, total=False):
    a51: int  # STATE_ON
    a71: int  # STATE_OFF

class StaticCameraType(TriggerType, total=False):
    a10: float  # DURATION
    a30: Any  # EASING
    a71: Required[int]  # TARGET_ID
    a85: float  # EASE_RATE
    a101: Any  # AXIS
    a110: bool  # EXIT
    a212: bool  # FOLLOW_GROUP
    a213: float  # FOLLOW_EASING
    a453: bool  # SMOOTH_VELOCITY
    a454: float  # VELOCITY_MOD
    a465: bool  # EXIT_INSTANT

class StopType(TriggerType, total=False):
    a51: Required[int]  # TARGET_ID
    a535: bool  # USE_CONTROL_ID
    a580: Any  # MODE

class TeleportType(TriggerType, total=False):
    a51: Required[int]  # TARGET_ID
    a55: bool  # SMOOTH_EASE
    a345: bool  # USE_FORCE
    a346: float  # FORCE
    a347: bool  # REDIRECT_FORCE
    a348: float  # FORCE_MIN
    a349: float  # FORCE_MAX
    a350: float  # FORCE_MOD
    a351: bool  # KEEP_OFFSET
    a352: bool  # IGNORE_X
    a353: bool  # IGNORE_Y
    a354: Any  # GRAVITY
    a443: bool  # ADDITIVE_FORCE
    a464: bool  # INSTANT_CAMERA
    a510: bool  # SNAP_GROUND
    a591: bool  # REDIRECT_DASH

class ExitPortalType(TeleportType, total=False):
    a350: float  # M_350

class PortalType(TeleportType, total=False):
    a54: float  # DISTANCE

class TimeType(TriggerType, total=False):
    a51: Required[int]  # TARGET_ID
    a80: int  # ITEM_ID
    a467: float  # START_TIME
    a468: bool  # DONT_OVERRIDE
    a469: bool  # IGNORE_TIMEWARP
    a470: float  # MOD
    a471: bool  # START_PAUSED
    a474: bool  # STOP

class TimeControlType(TriggerType, total=False):
    a80: int  # ITEM_ID
    a472: bool  # STOP

class TimeEventType(TriggerType, total=False):
    a51: Required[int]  # TARGET_ID
    a80: int  # ITEM_ID
    a473: float  # TARGET_TIME
    a475: bool  # MULTI_ACTIVATE

class ToggleType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a56: bool  # ACTIVATE_GROUP

class ToggleBlockType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a56: bool  # ACTIVATE_GROUP
    a445: bool  # CLAIM_TOUCH
    a504: bool  # SPAWN_ONLY

class TouchType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a81: bool  # HOLD_MODE
    a82: Any  # TOGGLE_MODE
    a89: bool  # DUAL_MODE
    a198: Any  # ONLY_PLAYER

class UiType(TriggerType, total=False):
    a51: int  # GROUP_ID
    a71: int  # UI_TARGET
    a385: Any  # REF_X
    a386: Any  # REF_Y
    a387: bool  # RELATIVE_X
    a388: bool  # RELATIVE_Y

class ZoomCameraType(TriggerType, total=False):
    a10: float  # DURATION
    a30: Any  # EASING
    a85: float  # EASE_RATE
    a371: float  # ZOOM

# End
